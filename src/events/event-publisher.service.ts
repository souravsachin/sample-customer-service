...
import { CustomerEventType, ZorbitEventEnvelope } from './customer.events';
...
export class EventPublisherService implements OnModuleInit, OnModuleDestroy {
   ...
   * @param eventType - e.g. 'customer.customer.created'
  ...
  async publish<T>(
    ...
    const envelope: ZorbitEventEnvelope<T> = {
      ...
      source: 'sample-customer-service',
      namespace,
      namespaceId,
      payload,
    };

    const topic = eventType.replace(/\./g, '-'); // customer.customer.created → customer-customer-created
